...
function Navbar() {
  ...
  const handleScroll = () => {
    ...
    if (lastScrollTop.current > currentScroll || currentScroll <= 0) {
      setIsVisible(true);
    } else {
      setIsVisible(false);
    }
  ...
  return (
    <nav
      className={`flex items-center z-[999] justify-center py-2 mt-5 px-5 bg-secondary rounded-full w-fit mx-auto border-1 border-tertiary fixed left-1/2 transform -translate-x-1/2 transition-transform duration-300 shadow-lg shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10 ${isVisible ? 'translate-y-0' : '-translate-y-16'}`}>
      <ul className="flex items-center gap-8 sm:gap-4 justify-center">
        <li>
          <a className="text-sm hover:text-quartery" href="#home">
            <span className={"hidden sm:block"}>Omar</span>
        ...
        <li>
          <a className="text-sm hover:text-quartery" href="#showcase">
